...
def run_map_scraper():
    with sync_playwright() as p:
        ...
        for p_idx in range(1, 4): 
            ...
            if p_idx < 3:
                page.evaluate(f"() => {{ const btns = Array.from(document.querySelectorAll('.pagination a')); for(let b of btns) if(b.innerText.trim() == '{p_idx+1}') b.click(); }}")
                time.sleep(2)
            else: break
        ...
        if all_data:
            ...
            for item in all_data:
                ...
                if tid in existing_ids:
                    ...
                    # 更新關鍵欄位：地址、類型(7)、網址、座標
                    for col_idx in [4, 7, 11, 12, 14, 15]: 
                        val = d[col_idx-1]
